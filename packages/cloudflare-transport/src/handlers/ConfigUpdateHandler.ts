...
export class ConfigUpdateHandler extends EventEmitter {
   ...
   */
  async handlePaths(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
   ...
   */
  async handleGitHubCredentials(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
      ...
      console.error("[ConfigUpdateHandler] Failed to update GitHub credentials:", error);
      return {
        status: 500,
        body: {
          success: false,
   ...
   */
  async handleLinearCredentials(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
      const credentials: LinearCredentialsUpdate = JSON.parse(body);

      // Validate required fields
      if (!credentials.token || !credentials.workspaceId || !credentials.workspaceName) {
        return {
          status: 400,
          body: {
            success: false,
      ...
      console.error("[ConfigUpdateHandler] Failed to update Linear credentials:", error);
      return {
        status: 500,
        body: {
          success: false,
   ...
   */
  async handleClaudeApiKey(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
      ...
      console.error("[ConfigUpdateHandler] Failed to update Claude API key:", error);
      return {
        status: 500,
        body: {
          success: false,
   ...
   */
  async handleRepositories(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
      ...
      console.log(`[ConfigUpdateHandler] Repositories updated: ${repositories.length} repositories`);
      ...
      console.error("[ConfigUpdateHandler] Failed to update repositories:", error);
      return {
        status: 500,
        body: {
          success: false,
   ...
   */
  async handleGetConfig(req: IncomingMessage, body: string): Promise<HandlerResult> {
    try {
      ...
      console.error("[ConfigUpdateHandler] Failed to get configuration:", error);
      return {
        status: 500,
        body: {
          success: false,
