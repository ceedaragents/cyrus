...
export class CloudflareTransportClient extends EventEmitter {
	private config: CloudflareTransportConfig;
	...
	constructor(config: CloudflareTransportConfig) {
		...
		this.config = {
			port: 3457,
			hostedUrl:
				process.env.CYRUS_HOSTED_URL || "https://cyrus-hosted.vercel.app",
			autoStart: true,
			...config,
		};
		...
		if (
			...
			this.config.customerId !== storedConfig.customerId
		) {
	...
	async start(): Promise<void> {
		...
		try {
			...
			if (!config.customerId) {
				throw new Error(
					"Customer ID is required. Please run 'cyrus set-customer-id' or provide it during initialization.",
				);
			}
			...
			if (!config.cloudflareToken) {
				console.log(
					"[CloudflareTransportClient] No Cloudflare token found, validating customer...",
				);
				const validation = await this.validateCustomer(config.customerId);

				if (!validation.success || !validation.cloudflareToken) {
					throw new Error(
						`Customer validation failed: ${validation.message || "No token received"}`,
					);
				}
			...
			console.log(
				"[CloudflareTransportClient] Transport client started successfully",
			);
	...
	private async startHttpServer(): Promise<void> {
		...
		webhookHandler.on("webhook", (webhook: LinearWebhook) => {
			this.emit("webhook", webhook);
		});

		// Register config update event forwarding
		configHandler.on("config:updated", (type: string) => {
		...
		this.httpServer.registerHandler(
			"/health",
			healthHandler.handleHealth.bind(healthHandler),
		);
		this.httpServer.registerHandler(
			"/status",
			healthHandler.handleStatus.bind(healthHandler),
		);
		this.httpServer.registerHandler(
			"/webhook",
			webhookHandler.handle.bind(webhookHandler),
		);
		this.httpServer.registerHandler(
			"/config",
			configHandler.handleGetConfig.bind(configHandler),
		);
		this.httpServer.registerHandler(
			"/config/paths",
			configHandler.handlePaths.bind(configHandler),
		);
		this.httpServer.registerHandler(
			"/config/github-credentials",
			configHandler.handleGitHubCredentials.bind(configHandler),
		);
		this.httpServer.registerHandler(
			"/config/linear-credentials",
			configHandler.handleLinearCredentials.bind(configHandler),
		);
		this.httpServer.registerHandler(
			"/config/claude-api-key",
			configHandler.handleClaudeApiKey.bind(configHandler),
		);
		this.httpServer.registerHandler(
			"/config/repositories",
			configHandler.handleRepositories.bind(configHandler),
		);
	 ...
	 */
	private async startTunnel(token: string): Promise<void> {
		...
		this.tunnel.on("connected", (url: string) => {
			this.emit("tunnel:ready", url);
		});

		this.tunnel.on("disconnected", (reason?: string) => {
	...
	private async validateCustomer(
		...
		try {
			const request: CustomerValidationRequest = {
				customerId,
			...
			const response = await fetch(
				`${this.config.hostedUrl}/api/validate-customer`,
				{
					method: "POST",
					headers: {
			...
			return result as CustomerValidationResponse;
		} catch (error) {
			console.error(
				"[CloudflareTransportClient] Customer validation error:",
				error,
			);
			throw error;
		}
	 ...
	 * Register tunnel URL with cyrus-hosted
	 */
	private async registerTunnelUrl(tunnelUrl: string): Promise<void> {
		try {
			const config = this.configManager.get();

			const response = await fetch(
				`${this.config.hostedUrl}/api/register-tunnel`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			console.log(
				"[CloudflareTransportClient] Tunnel URL registered with cyrus-hosted",
			);
		} catch (error) {
			console.error(
				"[CloudflareTransportClient] Failed to register tunnel URL:",
				error,
			);
