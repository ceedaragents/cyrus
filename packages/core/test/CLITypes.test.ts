...
import { createCLIIssue, CLIIssueData, CLILabelData } from '../src/issue-tracker/adapters/CLITypes';
...
describe('createCLIIssue', () => {
    it('should return labels when labelIds are present', async () => {
        ...
        const resolvedLabels: CLILabelData[] = [
            { id: 'label-1', name: 'Label 1', color: '#ff0000', isGroup: false, createdAt: new Date(), updatedAt: new Date() },
            { id: 'label-2', name: 'Label 2', color: '#0000ff', isGroup: false, createdAt: new Date(), updatedAt: new Date() }
        ];
