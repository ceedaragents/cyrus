...
export interface ConfigurationManagerEvents {
	...
	"config:reloaded": (config: EdgeWorkerConfig, changes: ConfigurationChanges) => void;
...
export class ConfigurationManager extends EventEmitter {
	...
	startWatching(): void {
		if (this.watcher) {
			console.warn("[ConfigurationManager] Already watching configuration file");
			return;
		}

		try {
			console.log(`[ConfigurationManager] Starting to watch: ${this.configPath}`);

			this.watcher = watch(this.configPath, (eventType) => {
				console.log(`[ConfigurationManager] File change detected: ${eventType}`);
			...
			console.error("[ConfigurationManager] Failed to start file watcher:", error);
	...
	private reloadFromDisk(): void {
		try {
			console.log("[ConfigurationManager] Reloading configuration from disk...");
			...
			console.error("[ConfigurationManager] Failed to reload configuration:", error);
			this.emit("config:error", error as Error);
		}
	}
	...
	async updateConfiguration(partial: Partial<EdgeWorkerConfig>): Promise<void> {
		try {
			console.log("[ConfigurationManager] Updating configuration programmatically");
			...
			console.error("[ConfigurationManager] Failed to update configuration:", error);
			throw error;
		}
	}
	...
	private async saveConfiguration(config: EdgeWorkerConfig): Promise<void> {
		try {
			...
			console.log(`[ConfigurationManager] Configuration saved to: ${this.configPath}`);
			...
			console.error("[ConfigurationManager] Failed to save configuration:", error);
			throw error;
		}
	}
		...
		if (config.repositories) {
			for (const repo of config.repositories) {
				if (!repo.id || typeof repo.id !== "string") {
					throw new Error(`Invalid repository configuration: missing or invalid id`);
				}
				if (!repo.name || typeof repo.name !== "string") {
					throw new Error(`Invalid repository configuration: missing or invalid name for ${repo.id}`);
				}
				if (!repo.repositoryPath || typeof repo.repositoryPath !== "string") {
					throw new Error(`Invalid repository configuration: missing or invalid repositoryPath for ${repo.id}`);
				}
				if (!repo.linearToken || typeof repo.linearToken !== "string") {
					throw new Error(`Invalid repository configuration: missing or invalid linearToken for ${repo.id}`);
				}
				if (!repo.linearWorkspaceId || typeof repo.linearWorkspaceId !== "string") {
					throw new Error(`Invalid repository configuration: missing or invalid linearWorkspaceId for ${repo.id}`);
				}
			}
		}
	...
	private detectChanges(
		...
		if (JSON.stringify(oldConfigWithoutRepos) !== JSON.stringify(newConfigWithoutRepos)) {
	 ...
	 */
	async updateRepository(repositoryId: string, updates: Partial<RepositoryConfig>): Promise<void> {
		const currentRepos = this.currentConfig.repositories || [];
