...
import { DefaultRunnerFactory } from "cyrus-agent-runner";
import {
	ClaudeRunner,
...
const BUILT_IN_PROMPT_TYPES = ["debugger", "builder", "scoper", "orchestrator"];
const BUILT_IN_PROMPT_TYPE_SET = new Set(BUILT_IN_PROMPT_TYPES);
/**
...
export class EdgeWorker extends EventEmitter {
	...
	async handleWebhook(webhook, repos) {
		...
		try {
			...
			if (isIssueAssignedWebhook(webhook)) {
				...
				if (process.env.CYRUS_WEBHOOK_DEBUG === "true") {
					console.log(
						`[handleWebhook] Unhandled webhook type: ${webhook.action} for repository ${repository.name}`,
					);
				}
			}
		} catch (error) {
			console.error(
				`[handleWebhook] Failed to process webhook: ${webhook.action} for repository ${repository.name}`,
				error,
			);
	 ...
	 */
	async createLinearAgentSession(
	...
	async handleIssueUnassigned(issue, repository) {
		...
		const nonClaudeSessionsToRemove = [];
		for (const [
			sessionId,
			metadata,
	...
	resolveRunnerSelection(labels, repository) {
		const match = (() => {
	...
	normalizeError(value, fallback = "Unknown error") {
	...
	buildPromptPathCandidates(promptPath) {
		if (!promptPath) {
			return [];
		}
	...
	async loadPromptTemplateFromPath(promptName, promptPath) {
	...
	async loadBuiltInPrompt(promptName) {
		try {
	...
	async determineSystemPromptFromLabels(labels, repository) {
		...
		for (const [promptName, config] of Object.entries(labelPrompts)) {
			if (!BUILT_IN_PROMPT_TYPE_SET.has(promptName) && config) {
				orderedEntries.push([promptName, config]);
			}
		}
		for (const [promptName, rawConfig] of orderedEntries) {
			...
			let promptResult;
			const promptRule = Array.isArray(rawConfig) ? undefined : rawConfig;
	 ...
	 */
	async buildMentionPrompt(issue, agentSession, attachmentManifest = "") {
		try {
			console.log(
...
${newComment ? `New comment to address:\n${newComment.body}\n\n` : ""}Please analyze this issue and help implement a solution. ${LAST_MESSAGE_MARKER}`;
	...
	async postThought(linearAgentActivitySessionId, repositoryId, body) {
		const linearClient = this.linearClients.get(repositoryId);
		if (!linearClient) {
			this.debugLog(
	...
	async postResponse(linearAgentActivitySessionId, repositoryId, body) {
		const linearClient = this.linearClients.get(repositoryId);
		if (!linearClient) {
			this.debugLog(
	...
	async postAction(linearAgentActivitySessionId, repositoryId, action, detail) {
		const linearClient = this.linearClients.get(repositoryId);
		if (!linearClient) {
			this.debugLog(
	...
	async postError(linearAgentActivitySessionId, repositoryId, body) {
		const linearClient = this.linearClients.get(repositoryId);
		if (!linearClient) {
			this.debugLog(
	...
	async markSessionComplete(linearAgentActivitySessionId, repositoryId) {
		this.debugLog(
