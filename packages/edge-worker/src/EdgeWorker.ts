...
export class EdgeWorker extends EventEmitter {
	...
	constructor(config: EdgeWorkerConfig, configManager?: ConfigurationManager) {
		...
		this.configManager = configManager || new ConfigurationManager(
			join(this.cyrusHome, "config.json"),
			config
		);
	...
	private async handleConfigurationReload(
		...
		console.log(`Modified repositories: ${changes.repositoriesModified.length}`);
		...
		for (const repo of changes.repositoriesAdded) {
			console.log(`[ConfigReload] Adding new repository: ${repo.name} (${repo.id})`);
			...
			if (this.isStarted) {
				console.log(`[ConfigReload] Creating NDJSON client for new repository: ${repo.name}`);
				...
				for (const [existingRepoId, client] of this.ndjsonClients.entries()) {
					...
					if (existingRepo?.linearToken === repo.linearToken) {
						...
						console.log(`[ConfigReload] Reusing existing NDJSON client (token matches)`);
						break;
					}
				}
				...
				if (!existingClient) {
					...
					const clientConfig = {
						...
						...(!this.config.baseUrl &&
							this.config.webhookBaseUrl && { webhookBaseUrl: this.config.webhookBaseUrl }),
					...
					try {
						...
						console.log(`[ConfigReload] Successfully connected new repository: ${repo.name}`);
					} catch (error) {
						console.error(
			...
			console.log(`[ConfigReload] ✅ Successfully added repository: ${repo.name}`);
		}

		// Handle modified repositories (apply updates to existing repos)
		for (const repo of changes.repositoriesModified) {
			console.log(`[ConfigReload] Updating repository: ${repo.name} (${repo.id})`);
			...
			if (existingClient) {
				...
				console.log(`[ConfigReload] Note: Some changes (like token updates) require restart for full effect`);
			}
		...
		for (const repo of changes.repositoriesRemoved) {
			console.log(`[ConfigReload] Queuing repository for removal: ${repo.name} (${repo.id})`);
			...
			if (activeSessions.length > 0) {
				...
				console.log(`[ConfigReload] Removing repository immediately: ${repo.name}`);
		...
		if (changes.otherChanges) {
			console.log("[ConfigReload] ℹ️  Global configuration settings were updated");
			console.log("[ConfigReload] Note: Global setting changes apply to new sessions");
		}
