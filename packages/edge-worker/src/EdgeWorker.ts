...
${input.userComment}
	...
	// ========================================================================
	// TEST-ONLY METHODS
	// These methods are exposed for testing purposes only and should not be
	// used in production code. They allow tests to inject mock dependencies.
	// ========================================================================

	/**
	 * @internal - Set the issue tracker instance for testing
	 * @param issueTracker - The issue tracker mock to inject
	 */
	setIssueTrackerForTesting(issueTracker: IIssueTrackerService): void {
		this.issueTracker = issueTracker;
	}

	/**
	 * @internal - Get the issue tracker instance for testing
	 * @returns The current issue tracker instance
	 */
	getIssueTrackerForTesting(): IIssueTrackerService {
		return this.issueTracker;
	}

	/**
	 * @internal - Set the agent session manager instance for testing
	 * @param manager - The agent session manager mock to inject
	 */
	setAgentSessionManagerForTesting(manager: AgentSessionManager): void {
		this.agentSessionManager = manager;
	}

	/**
	 * @internal - Get the agent session manager instance for testing
	 * @returns The current agent session manager instance
	 */
	getAgentSessionManagerForTesting(): AgentSessionManager {
		return this.agentSessionManager;
	}

	/**
	 * @internal - Set the active webhook count for testing
	 * @param count - The count to set
	 */
	setActiveWebhookCountForTesting(count: number): void {
		this.activeWebhookCount = count;
	}

	/**
	 * @internal - Get the active webhook count for testing
	 * @returns The current active webhook count
	 */
	getActiveWebhookCountForTesting(): number {
		return this.activeWebhookCount;
	}

	/**
	 * @internal - Get the repositories map for testing
	 * @returns The repositories map
	 */
	getRepositoriesForTesting(): Map<string, RepositoryConfig> {
		return this.repositories;
	}

	/**
	 * @internal - Get the childToParentAgentSession map for testing
	 * @returns The child to parent agent session map
	 */
	getChildToParentAgentSessionForTesting(): Map<string, string> {
		return this.childToParentAgentSession;
	}
}
