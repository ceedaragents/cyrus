 ...
 */
export class AnthropicAgentRunner implements IAgentRunner {
	...
	private completeHandlers: Array<
		(result: AgentResult) => void | Promise<void>
	> = [];
	private errorHandlers: Array<(error: Error) => void | Promise<void>> = [];
	private initialized = false;

	// Bound event handlers for proper cleanup
	private boundMessageHandler?: (sdkMessage: SDKMessage) => void;
	private boundErrorHandler?: (error: Error) => void;
	private boundCompleteHandler?: (messages: SDKMessage[]) => void;
	...
	private setupEventHandlers(): void {
		...
		this.boundMessageHandler = (sdkMessage: SDKMessage) => {
		...
		this.boundCompleteHandler = (messages: SDKMessage[]) => {
	 ...
	 */
	private async emitMessage(message: AgentMessage): Promise<void> {
	 ...
	 */
	private async emitComplete(result: AgentResult): Promise<void> {
	 ...
	 */
	async execute(prompt: AgentPrompt): Promise<AgentSession> {
		if (!this.initialized) {
			throw new Error(
		...
		if (prompt.context) {
			...
			if (prompt.context.workingDirectory) {
				(this.claudeRunner as any).config.workingDirectory =
					prompt.context.workingDirectory;
			}
			if (prompt.context.environment) {
				(this.claudeRunner as any).config.environment =
					prompt.context.environment;
			}
			if (prompt.context.systemPrompt) {
				(this.claudeRunner as any).config.systemPrompt =
	 ...
	 */
	private createSession(_prompt: AgentPrompt): AgentSession {
		const sessionId = `session-${Date.now()}`;
		const messages: AgentMessage[] = [];
		let resultResolver: (result: AgentResult) => void;
		let resultRejecter: (error: Error) => void;
		...
		// Handle completion
		const completeHandler = (result: AgentResult) => {
