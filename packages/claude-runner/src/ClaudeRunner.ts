...
export class ClaudeRunner extends EventEmitter implements IAgentRunner {
	...
	/**
	 * Resolve SDK auth settings. Uses apiKeyHelper to let the SDK
	 * obtain credentials on demand from <cyrusHome>/.env.
	 * Auto-creates the helper script if it doesn't exist.
	 */
	private resolveAuthSettings():
	...
	/**
	 * Create the auth helper script at the given path.
	 * The script reads from the Cyrus .env file (path baked in at generation time),
	 * then falls back to shell profiles.
	 */
	private ensureAuthHelper(helperPath: string): void {
		try {
			const dir = join(helperPath, "..");
			mkdirSync(dir, { recursive: true });

			// Resolve the env file path — use configured envFile or default to <cyrusHome>/.env
			...
			const lines = [
				...
				"# Auto-generated by ClaudeRunner. Do not edit manually.",
				"#",
				"# Source precedence: CYRUS_ENV_FILE env var > baked-in path > shell profiles",
				"# Token precedence: ANTHROPIC_API_KEY > CLAUDE_CODE_OAUTH_TOKEN > ANTHROPIC_AUTH_TOKEN",
				"",
			...
			this.logger.info(`Created auth helper at ${helperPath}`);
		} catch (error) {
			this.logger.warn("Failed to create auth helper:", error);
		}
	}
