...
### Changed
- `SlackEventTransport.getSlackBotToken()` now reads `SLACK_BOT_TOKEN` from `process.env` as the primary token source, falling back to the `X-Slack-Bot-Token` request header for backwards compatibility during the CYHOST transition. Logs a warning when using the header fallback. Added 6 new tests covering env var precedence, header fallback, warning logging, and missing token scenarios. ([CYPACK-824](https://linear.app/ceedar/issue/CYPACK-824), [#876](https://github.com/ceedaragents/cyrus/pull/876))
