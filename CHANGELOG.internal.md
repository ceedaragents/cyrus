...
### Fixed
- Fixed mislabeled `routingMethod` in `RepositoryRouter`: existing-active-session routing (Priority 0) was returning `routingMethod: "workspace-fallback"`, which is the same label used for the no-`workspaceId` fallback path. Added a distinct `"existing-session"` member to the `routingMethod` union and a `"Existing session"` display name in `ActivityPoster.postRoutingActivity`'s `methodDisplayMap`, so the Linear "Routing" activity and EdgeWorker logs now distinguish session-cache routing from workspace-id-missing fallback. ([ENG-45](https://linear.app/cyrusagenttesting/issue/ENG-45/find-a-small-improvemtnet))
