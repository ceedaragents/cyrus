...
### Changed
- Removed redundant `sessionRepositories: Map<string, string>` from EdgeWorker. All ~14 usage sites now read directly from `session.repositories[0]?.repositoryId`; reverse-lookup in repo deletion uses `session.repositories.some()` for correct multi-repo matching. Added TODO comments documenting `activitySinks` multi-workspace limitation and `issueTrackers` composite key for future multi-platform support. ([CYPACK-968](https://linear.app/ceedar/issue/CYPACK-968), [#992](https://github.com/ceedaragents/cyrus/pull/992))
