...
### Changed
- Threaded `linearWorkspaceId` from `webhook.organizationId` (Linear-native source) through EdgeWorker webhook-driven paths, replacing `requireLinearWorkspaceId(repo)` calls. Reduced usage from 45 to 24 calls. Renamed all `workspaceId` variables to `linearWorkspaceId` for clarity. Decoupled `buildMcpConfig` and `buildCyrusToolsMcpContextId` from `RepositoryConfig` (now take `repoId` string). Eliminated `firstRepo` pattern in Slack MCP setup. ([CYPACK-966](https://linear.app/ceedar/issue/CYPACK-966), [#990](https://github.com/ceedaragents/cyrus/pull/990))
