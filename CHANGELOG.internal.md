...
- Added `PreToolUse` hooks to `buildAgentRunnerConfig()` in EdgeWorker for context-mode tool guidance. When `runnerType === 'claude'` and context-mode is enabled, hooks intercept `Bash` commands with `curl`/`wget` and `WebFetch` calls to suggest `context-mode:fetch_and_index` and `context-mode:search` alternatives, preserving context window space without blocking tool execution. Exported `PreToolUseHookInput` from `cyrus-claude-runner`. ([CYPACK-874](https://linear.app/ceedar/issue/CYPACK-874), [#926](https://github.com/ceedaragents/cyrus/pull/926))
- Added `contextModeLevel?: "full" | "light" | "disabled"` field to `SubroutineDefinition` for per-subroutine context-mode control. Wired through `buildAgentRunnerConfig()` so `"disabled"` excludes both context-mode MCP server and PreToolUse hooks, `"light"` includes MCP server but skips PreToolUse hooks, and `"full"` (default) enables full context-mode. Updated subroutine registry: `gitCommit` and `ghPr` set to `"light"`, `changelogUpdate` set to `"disabled"`. ([CYPACK-875](https://linear.app/ceedar/issue/CYPACK-875), [#927](https://github.com/ceedaragents/cyrus/pull/927))
- Added `SessionStart` hook to `buildAgentRunnerConfig()` in EdgeWorker that injects `additionalContext` guidance at the start of each session when context-mode is enabled (`contextModeHooksEnabled`). The hook teaches Claude about all six context-mode MCP tools (`execute`, `execute_file`, `batch_execute`, `fetch_and_index`, `index`, `search`) and when to prefer them over standard tools. ([CYPACK-876](https://linear.app/ceedar/issue/CYPACK-876), [#928](https://github.com/ceedaragents/cyrus/pull/928))
- Added context-mode session metrics logging to `buildAgentRunnerConfig()` in EdgeWorker. Logs at `info` level when context-mode is enabled (with subroutine level and hooks status), and at `debug` level when disabled (with runner type, repo config, and level). Provides visibility into context-mode usage across Cyrus sessions. ([CYPACK-877](https://linear.app/ceedar/issue/CYPACK-877), [#929](https://github.com/ceedaragents/cyrus/pull/929))
