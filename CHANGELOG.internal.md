...
### Added
- Wired user-configured MCP support into Slack chat sessions. Added `McpConfigService.extractServerNamesFromConfigPaths()` to read `.mcp.json` files and extract server name keys. Extended `ToolPermissionResolver.buildChatAllowedTools()` to accept `userMcpServerNames` parameter for user-configured MCP tool prefixes. Updated `ChatRunnerConfigInput`, `ChatSessionHandlerDeps`, and `RunnerConfigBuilder.buildChatConfig()` to carry `mcpConfigPath` and `userMcpServerNames`. `EdgeWorker.registerSlackEventTransport()` now computes merged `mcpConfigPath` from all repositories and passes both config path and extracted server names to `ChatSessionHandler`. ([CYPACK-982](https://linear.app/ceedar/issue/CYPACK-982), [#1006](https://github.com/ceedaragents/cyrus/pull/1006))
