...
### Changed
- Renamed `createLinearAgentSession` to `createCyrusAgentSession` across `packages/edge-worker` (2 source files, 15 test files). The "Linear" prefix was a leaky abstraction — the method creates a Cyrus agent session and already accepts a `platform` param for github, slack, etc. ([CYPACK-969](https://linear.app/ceedar/issue/CYPACK-969), [#994](https://github.com/ceedaragents/cyrus/pull/994))
