...
### Changed
- Updated `@anthropic-ai/claude-agent-sdk` to v0.2.49 and `@anthropic-ai/sdk` to v0.78.0. SDK v0.2.49 adds model capability fields (`supportsEffort`, `supportedEffortLevels`, `supportsAdaptiveThinking`), permission suggestion population during safety check ask responses, and `ConfigChange` hook events. SDK v0.78.0 adds top-level cache control and fixes AWS Bedrock credential race conditions. ([CYPACK-830](https://linear.app/ceedar/issue/CYPACK-830), [#882](https://github.com/ceedaragents/cyrus/pull/882))
