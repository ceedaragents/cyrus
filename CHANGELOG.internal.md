...
### Changed
- Improved Task tool formatting with batching: TaskCreate/TaskUpdate calls are now debounce-batched (300ms) in AgentSessionManager and posted as a single consolidated checklist activity instead of individual items. Added `formatTaskBatch()` to IMessageFormatter interface and both ClaudeMessageFormatter and GeminiMessageFormatter. Added `pendingTaskBatch` map with timer-based flush in AgentSessionManager. TaskGet and TaskList remain unbatched. TaskCreate bolds subjects, TaskUpdate/TaskGet show cached task subjects via `taskSubjectsByTaskId`. ([CYPACK-794](https://linear.app/ceedar/issue/CYPACK-794), [#845](https://github.com/ceedaragents/cyrus/pull/845))
