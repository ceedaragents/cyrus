...
### Added
- Added repository-level container execution primitives for self-hosted Cyrus: `hostPaths`, `verification.mode = "ephemeral_container"`, and `agentExecution.mode = "persistent_issue_container"`. This includes host/worktree path mapping, Docker-based verification execution, persistent issue-container lifecycle management, and F1 wiring for both execution modes.
- Added Claude issue-container execution via a bridge process in `cyrus-claude-runner`, allowing the Claude Agent SDK query loop to run inside the issue container while Cyrus keeps orchestration, activity posting, and stop handling on the host.
- Added a Codex-enabled F1 image at `apps/f1/images/codex-test/Dockerfile`, plus F1 drive reports covering ephemeral verification and persistent issue-container execution.

### Fixed
- Synced CLI/F1 agent session completion back into the in-memory issue tracker when Cyrus finalizes a session, so `view-session` now reports `complete` instead of leaving finished runs at `active`.
- Sanitized CLI-generated branch names for issue titles with punctuation such as `Question: ...`, preventing invalid branch names from breaking `git worktree add -b ...` in F1 and CLI-mode testing.
