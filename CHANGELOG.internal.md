...
### Changed
- Updated `@anthropic-ai/claude-agent-sdk` from `^0.2.63` to `^0.2.76` in `packages/core`, `packages/claude-runner`, and `packages/simple-agent-runner`. Fixed import path in `core/src/agent-runner-types.ts`: removed explicit `.d.ts` extension from the `sdk-tools` subpath import, which broke TypeScript resolution against the new package exports map. `@anthropic-ai/sdk` was already at the latest `^0.78.0`. ([CYPACK-970](https://linear.app/ceedar/issue/CYPACK-970), [#995](https://github.com/ceedaragents/cyrus/pull/995))
