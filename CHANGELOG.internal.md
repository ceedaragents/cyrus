...
### Changed
- `SlackEventTransport.getSlackBotToken()` now reads `SLACK_BOT_TOKEN` exclusively from `process.env` with no header fallback. The `X-Slack-Bot-Token` request header is no longer used. Added `mcpConfig` support to `ChatSessionHandler`/`ChatSessionHandlerDeps` so Slack sessions receive Linear and cyrus-tools MCP servers (using the first repository's Linear token) with proper tool permissions (`mcp__linear`, `mcp__cyrus-tools`). ([CYPACK-824](https://linear.app/ceedar/issue/CYPACK-824), [#876](https://github.com/ceedaragents/cyrus/pull/876))
