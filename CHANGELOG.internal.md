...
### Added
- Wired user-configured MCP support into Slack chat sessions. Added `McpConfigService.extractServerNamesFromConfigPaths()` (exposed via `IMcpConfigProvider`) to read `.mcp.json` files and extract server name keys. Extended `ToolPermissionResolver.buildChatAllowedTools()` to accept `userMcpServerNames` parameter for user-configured MCP tool prefixes. `RunnerConfigBuilder.buildChatConfig()` now derives `mcpConfigPath` and server names internally from an optional `repository` reference, matching the `buildIssueConfig()` pattern (SRP). `EdgeWorker.registerSlackEventTransport()` passes the first repo to `ChatSessionHandler`, which forwards it to `RunnerConfigBuilder`. ([CYPACK-982](https://linear.app/ceedar/issue/CYPACK-982), [#1006](https://github.com/ceedaragents/cyrus/pull/1006))
