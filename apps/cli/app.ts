...
async function reloadConfigCommand() {
	...
	console.log("─".repeat(50));
	console.log("⚠️  Note: This command is only useful when Cyrus is not running.");
	console.log("When Cyrus is running, configuration changes are detected automatically.\n");
	...
	try {
		...
		if (config.repositories) {
			config.repositories.forEach((repo, index) => {
				console.log(`   ${index + 1}. ${repo.name} (${repo.linearWorkspaceName || repo.linearWorkspaceId})`);
			});
		}
		console.log("\n💡 To apply these changes while Cyrus is running:");
		console.log("   The changes will be detected and applied automatically!");
		console.log("   No action needed - just edit ~/.cyrus/config.json and save.");
	} catch (error) {
