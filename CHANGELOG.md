...
### Security
- **Expanded sandbox defaults for node-based package managers** — The sandbox now grants read access to `~/.gitconfig` and `~/.config/gh/hosts.yml`, and read+write access to the standard caches/stores used by npm, yarn, pnpm, bun, deno, and node-gyp (both macOS and Linux layouts). `TMPDIR` is also pointed at a Cyrus-managed directory (`<cyrusHome>/tmp`) that is always inside the write allowlist, so Bun and similar tools that need a writable tmp dir work out of the box without loosening the overall sandbox. ([CYPACK-1128](https://linear.app/ceedar/issue/CYPACK-1128))
