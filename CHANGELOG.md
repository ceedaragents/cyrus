...
### Fixed
- **Stop signal no longer posts a spurious error to Linear** — When a user sent a "stop" signal to interrupt Cyrus mid-turn, the underlying Claude SDK surfaced an internal `[ede_diagnostic]` result as a session error, which was then posted to Linear as a visible error activity ("Claude Code returned an error result: [ede_diagnostic] result_type=user last_content_type=n/a stop_reason=null"). Interrupt-triggered diagnostics are now recognized and treated as a normal user-initiated stop — the interrupt acknowledgment is still posted, but the diagnostic error is not. ([CYPACK-1129](https://linear.app/ceedar/issue/CYPACK-1129), [#1156](https://github.com/cyrusagents/cyrus/pull/1156))
