...
- Fixed `cyrusStatus` stuck in "running" when session completed. Updated `completeSession` to use proper state machine transitions (`ResultReceived` → `CleanupComplete`) instead of deprecated `updateSessionStatus`. Added fallback handling for edge cases where session state is out of sync ([CYPACK-536](https://linear.app/ceedar/issue/CYPACK-536))
