...
### Security
- **Patched open Dependabot advisories** — Bumped `@anthropic-ai/sdk` direct dep in `cyrus-claude-runner` to `^0.91.1` (fixes [GHSA-p7fg-763f-g4gf](https://github.com/anthropics/anthropic-sdk-typescript/security/advisories/GHSA-p7fg-763f-g4gf): insecure default file permissions in the local filesystem memory tool) and added `pnpm.overrides` for `@anthropic-ai/sdk` (>=0.91.1) and `uuid` (>=14.0.0, fixes [GHSA-w5hq-g745-h8pq](https://github.com/advisories/GHSA-w5hq-g745-h8pq) for transitive deps via `@google/gemini-cli-core`). `pnpm audit` now reports zero advisories. ([CYPACK-1153](https://linear.app/ceedar/issue/CYPACK-1153), [#1173](https://github.com/ceedaragents/cyrus/pull/1173))
