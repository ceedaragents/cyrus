...
- Updated `@anthropic-ai/claude-agent-sdk` from v0.1.69 to v0.1.72 to maintain parity with Claude Code v2.0.72. This update includes fixes for `/context` command respecting custom system prompts, non-streaming single-turn queries closing immediately, V2 session API method rename from `receive()` to `stream()`, zod v4.0.0 peer dependency support, AskUserQuestion tool support, Windows console window fix, and spawn message stderr fix. See the [Claude Agent SDK changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md#0172) for full details. ([CYPACK-629](https://linear.app/ceedar/issue/CYPACK-629))
