...
- **Updated Claude SDK dependencies** - Updated `@anthropic-ai/claude-agent-sdk` to v0.2.56 and `@anthropic-ai/sdk` to v0.78.0. New SDK features include `listSessions()` for discovering past sessions, `task_progress` events for real-time background agent progress reporting, `ConfigChange` hook events for security auditing, model capability fields (`supportsEffort`, `supportedEffortLevels`, `supportsAdaptiveThinking`), and top-level automatic cache control in the Anthropic SDK. See [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for full details. ([CYPACK-847](https://linear.app/ceedar/issue/CYPACK-847), [#899](https://github.com/ceedaragents/cyrus/pull/899))
