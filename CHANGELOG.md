...
### Added
- **Remote Claude session transcripts** — When both `CYRUS_APP_URL` and `CYRUS_API_KEY` are set, Cyrus now mirrors every Claude session transcript to the hosted Cyrus control plane (in addition to the local JSONL on disk). This lets sessions be inspected or resumed from any host, even after the ephemeral worktree is torn down. The transport speaks the Claude Agent SDK's `SessionStore` contract and passes the full 13-check behavioral conformance suite from the upstream SDK. ([CYPACK-1121](https://linear.app/ceedar/issue/CYPACK-1121))
