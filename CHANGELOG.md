...
### Added
- **Optional Sentry error tracking** — When a DSN is configured, all `logger.error(...)` calls across the codebase (Claude Code/runner errors, edge-worker failures, webhook transport errors, persistence errors, uncaught exceptions, unhandled rejections) are reported to Sentry so the team can spot and fix bugs faster. Set `CYRUS_SENTRY_DSN` to enable, or set `CYRUS_SENTRY_DISABLED=1` to opt out (also disables the bundled default DSN once it ships). Override the environment tag with `CYRUS_SENTRY_ENVIRONMENT`, sample with `CYRUS_SENTRY_SAMPLE_RATE` (0.0–1.0). Set `CYRUS_TEAM_ID` to enrich every event with a `team_id` tag for per-tenant filtering. Outgoing events are scrubbed for token-shaped strings and sensitive keys before transmission, and grouped by a stable fingerprint so log messages with embedded IDs/paths don't fragment into one issue per occurrence. No telemetry is sent unless a DSN is present. ([CYPACK-1142](https://linear.app/ceedar/issue/CYPACK-1142))
