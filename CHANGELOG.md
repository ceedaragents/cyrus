...
### Fixed
- Posting an agent activity to Linear now costs one API request instead of three. Both activity-posting paths read the newly created activity back through `AgentActivityPayload.agentActivity`, an unmemoized Linear SDK getter that issues a fresh query on every property access, and both touched it twice while consuming only its `id`. The two extra requests per activity tripled Cyrus's consumption of the Linear API budget and drove rate limiting on busy sessions; a read-back that failed also left an unhandled promise rejection behind. Activity ids now come from `agentActivityId`, which the mutation response already carries. ([#1448](https://github.com/cyrusagents/cyrus/pull/1448))
