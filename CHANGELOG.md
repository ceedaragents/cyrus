...
- **Effort level support** — Control how much reasoning effort Claude puts into its responses via Linear labels or issue description tags. Supports `[effort=low|medium|high|xhigh|max]` description tags, flat labels (`effort-max`, `max-effort`), and Linear label groups (`Effort Level/max`). Claude runner only. ([VAN-106](https://linear.app/van4oza/issue/VAN-106), [#1119](https://github.com/ceedaragents/cyrus/pull/1119))

### Changed
- **Claude Opus 4.7 is now the default for the `opus` alias** — Bumped SDK to v0.2.112 which resolves the `opus` model alias to `claude-opus-4-7`. The previous SDK version (0.2.111) still resolved to `claude-opus-4-6`. ([VAN-106](https://linear.app/van4oza/issue/VAN-106), [#1119](https://github.com/ceedaragents/cyrus/pull/1119))
- **Updated `@anthropic-ai/claude-agent-sdk` to v0.2.112 and `@anthropic-ai/sdk` to v0.90.0** — Refreshed both Anthropic SDK dependencies to their latest versions. Also updated tool allowance lists to match the new SDK: adds `LSP`, `ToolSearch`, and `PushNotification`. See the [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for full details. ([CYPACK-1090](https://linear.app/ceedar/issue/CYPACK-1090), [#1113](https://github.com/ceedaragents/cyrus/pull/1113))
