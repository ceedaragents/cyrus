...
### Added
- **Email-synced Linear comment threads now feed the agent session** — When Linear mirrors a comment thread to an email chain (identified by the auto-generated root comment "This comment thread is synced to a corresponding email chain. All replies are displayed in both locations."), replies in that thread are now injected into the most recently created agent session on the issue. Cyrus's own comments are filtered out to prevent feedback loops; override the bot email detection with `CYRUS_LINEAR_BOT_EMAIL` if you use a custom OAuth app. ([CYPACK-1134](https://linear.app/ceedar/issue/CYPACK-1134), [#1159](https://github.com/cyrusagents/cyrus/pull/1159))
