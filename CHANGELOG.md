...
### Security
- **Patched `uuid` advisory GHSA-w5hq-g745-h8pq** — Added a root `pnpm.overrides` entry forcing `uuid >= 14.0.0` to resolve the medium-severity buffer bounds-check advisory affecting transitive `uuid` versions pulled in by `@google/gemini-cli-core` (devDependency). `pnpm audit` now reports zero advisories. ([CYPACK-1147](https://linear.app/ceedar/issue/CYPACK-1147), [#1167](https://github.com/ceedaragents/cyrus/pull/1167))
