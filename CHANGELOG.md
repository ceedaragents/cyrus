...
### Added
- **Native OpenTelemetry logs support** — Configure an OTLP/HTTP endpoint via `telemetry` in `~/.cyrus/config.json` (or standard `OTEL_EXPORTER_OTLP_*` env vars) and every log record and session lifecycle event is mirrored to your observability backend alongside the usual console output. Operational events (`session.started`, `session.resumed`, `session.completed`, `session.stopped`, `session.failed`) always flow through regardless of `minLogLevel`, so dashboards see full session signal even when verbose logs are filtered out. Sensitive keys (tokens, secrets, authorization headers) are redacted before export. Graceful shutdown flushes pending records under a bounded timeout. ([CYPACK-1113](https://linear.app/ceedar/issue/CYPACK-1113))
