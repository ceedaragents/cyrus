...
- **Claude sessions close their streaming prompt on `result` unless warm mode is enabled** — When `CYRUS_ENABLE_WARM_SESSIONS` is unset, the in-session streaming prompt is now completed when the SDK emits a `result` message, so the subprocess shuts down at turn end (restores the pre-warm-sessions behavior). When `CYRUS_ENABLE_WARM_SESSIONS=1`, the streaming prompt stays open so follow-up messages keep the session warm.
