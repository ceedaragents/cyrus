...
### Added
- **Optional Sentry error tracking** — When a DSN is configured, all `logger.error(...)` calls across the codebase (Claude Code/runner errors, edge-worker failures, webhook transport errors, persistence errors, uncaught exceptions, unhandled rejections) are reported to Sentry so the team can spot and fix bugs faster. Set `CYRUS_SENTRY_DSN` to enable, or set `CYRUS_SENTRY_DISABLED=1` to opt out (also disables the bundled default DSN once it ships). Override the environment tag with `CYRUS_SENTRY_ENVIRONMENT`, sample with `CYRUS_SENTRY_SAMPLE_RATE` (0.0–1.0). Set `CYRUS_TEAM_ID` to enrich every event with a `team_id` tag (and a structured `cyrus` context block alongside `linear_workspace`/`deployment_id` if those env vars are set) for per-tenant filtering and grouped display in the Sentry UI. Setting `CYRUS_TEAM_ID` also opts the install into [Sentry Logs](https://docs.sentry.io/product/explore/logs/) forwarding: all `WARN`/`ERROR` logs and major lifecycle events (session started/resumed/completed/stopped, Claude session ID assigned, message emitted, webhook received, Claude query options) are forwarded as structured logs tagged with `team_id`, `component`, and active session/issue/Claude-session identifiers — debug/info logs stay local to keep volume bounded. Issue capture is independent of `CYRUS_TEAM_ID`, so errors still surface even on installs that haven't opted into log forwarding. The Sentry SDK's own internal debug output is gated separately on `CYRUS_SENTRY_DEBUG` to avoid flooding the terminal when you debug the app. Outgoing events **and** logs are scrubbed for token-shaped strings and sensitive keys before transmission (including breadcrumbs from console output), and grouped by a stable fingerprint so log messages with embedded IDs/paths don't fragment into one issue per occurrence. No telemetry is sent unless a DSN is present. ([CYPACK-1142](https://linear.app/ceedar/issue/CYPACK-1142))
