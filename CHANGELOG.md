...
### Added
- **Figma MCP integration via OAuth** — Cyrus can now connect to Figma's MCP server (`https://mcp.figma.com/mcp`) using OAuth authentication. Run `cyrus self-auth-figma` to authenticate, and the Figma MCP will be automatically available in all sessions. Requires `FIGMA_CLIENT_ID` and `FIGMA_CLIENT_SECRET` environment variables. ([CYPACK-1070](https://linear.app/ceedar/issue/CYPACK-1070), [#1098](https://github.com/ceedaragents/cyrus/pull/1098))
