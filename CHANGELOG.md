...
### Added
- **Email-synced Linear comment threads now feed the agent session** — When Linear mirrors a comment thread to an email chain (identified by the auto-generated root comment "This comment thread is synced to a corresponding email chain. All replies are displayed in both locations."), replies in that thread are now injected into the most recently created agent session on the issue. Set `CYRUS_LINEAR_BOT_EMAIL` to your Cyrus Linear OAuth app user email (e.g. `<uuid>@oauthapp.linear.app`) so Cyrus's own thread replies are filtered out and don't feed back into the session; the feature is a no-op until this env var is set. ([CYPACK-1134](https://linear.app/ceedar/issue/CYPACK-1134), [#1159](https://github.com/cyrusagents/cyrus/pull/1159))
- **Strict tool permission enforcement for env-bound sessions** — Fixes a long-standing bug where Cyrus's `canUseTool` callback rubber-stamped any tool the SDK asked about (i.e., any tool not in `allowedTools`), making `allowedTools` lists silently advisory. Now: any session bound to an environment automatically runs in strict mode — Cyrus auto-extends `disallowedTools` with every built-in tool name not represented in `allowedTools` (the SDK actively enforces `disallowedTools` even for "safe" tools like Glob/Grep/Bash that bypass `canUseTool`). `AskUserQuestion` is always preserved. Sessions with no environment bound keep the legacy permissive behavior. Environments can opt back into the legacy behavior by setting `"strictToolPermissions": false`. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
- **Linear activity for environment binding** — When an issue's `env=<name>` matcher resolves to a configured environment, Cyrus now posts an `**Environment** \`<name>\`` thought activity to the Linear timeline summarizing what the environment scoped: tools, MCP config, sandbox, plugins/skills, settings sources, env variables, accepted inline overrides, read-only repos, worktrees, and the home-directory read restriction. Only fields the environment actually customized are listed, so the entry stays scannable. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
- **Environment-controlled home-directory read restrictions** — `restrictHomeDirectoryReads` (boolean) on the environment config now lets you opt out of Cyrus's home-directory enumeration that adds every top-level entry under `~/` (other than worktree ancestors) to `disallowedTools` as `Read(<path>/**)`. Defaults to `true` (current safety behavior — blocks SSH keys, dotfiles, credentials). Set `false` for environments that intentionally need wider read access, e.g. cross-repo research sessions. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
- **Fully isolated environments** — Set `"isolated": true` on an environment config to opt out of all implicit merging with repository defaults, label prompts, default tool lists, auto-discovered skills, dynamic Linear/Slack MCP servers, the Stop and screenshot hooks, the Chrome extra-arg, the file-based settings sources default, and the hardcoded sandbox filesystem rules. The session is built solely from the environment config; fields the env omits become empty rather than inheriting from elsewhere. Two safety exceptions remain enforced at runtime: the worktree path is always added to sandbox `allowWrite` so the agent can persist its work, and the egress-proxy CA-cert env vars are always layered on top of `env` so MITM TLS keeps working. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
- **Scoped environments for agent sessions** — You can now define reusable environment bundles as JSON files in `~/.cyrus/environments/<name>.json` and bind one to a session by writing `env=<name>` or `[env=<name>]` in an issue description. An environment can scope the session's system prompt, allowed/disallowed tools, MCP config paths, sandbox filesystem permissions, environment variables, plugins, skills, and which Claude file-based settings sources (user/project/local) get merged into the agent (defaults preserved when `claudeSettingSources` is omitted; set `[]` to fully isolate). It can also independently specify `repositories` (extra repos granted read-only access via their on-disk paths) and `gitWorktrees` (0 or more repo names for which git worktrees should be created — defaults to the routed repositories when omitted). Issue descriptions can also tune env variables inline — `env=test$FEATURE_FLAG=1,$CYRUS_REMOTE=true` — but only for keys listed in the environment's `allowInlineOverrides` allowlist; keys not in the list are silently dropped. Once bound, the environment and any accepted inline overrides persist across restarts. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
