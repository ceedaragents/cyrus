...
### Changed
- **Updated Claude SDK dependencies** - Updated `@anthropic-ai/claude-agent-sdk` to v0.2.49 and `@anthropic-ai/sdk` to v0.78.0, adding model capability discovery (`supportsEffort`, `supportedEffortLevels`, `supportsAdaptiveThinking`), permission suggestions during safety checks, `ConfigChange` hook events, top-level cache control, and AWS Bedrock credential resolution fixes. See [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for details. ([CYPACK-830](https://linear.app/ceedar/issue/CYPACK-830), [#882](https://github.com/ceedaragents/cyrus/pull/882))
