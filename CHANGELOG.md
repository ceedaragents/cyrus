...
### Changed
- **Updated `@anthropic-ai/claude-agent-sdk` to v0.2.119 and `@anthropic-ai/sdk` to v0.91.0** — Bumps the bundled Claude Code binary to v2.1.119 and the Anthropic SDK. The `LSP` tool is no longer emitted by Claude Code's init block and has been removed from the available tools list in `config.ts`. See [SDK changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for details. ([CYPACK-1132](https://linear.app/ceedar/issue/CYPACK-1132), [#1158](https://github.com/cyrusagents/cyrus/pull/1158))
