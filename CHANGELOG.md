...
- Updated `@anthropic-ai/claude-agent-sdk` from v0.1.69 to v0.1.73 to maintain parity with Claude Code v2.0.73. This update includes fixed `/context` command behavior to respect custom system prompts, improved non-streaming performance for single-turn queries, renamed V2 session API method from `receive()` to `stream()`, and fixed a bug where Stop hooks would not consistently run due to "Stream closed" error. See the [Claude Agent SDK v0.1.73 changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md#0173) for full details. ([CYPACK-618](https://linear.app/ceedar/issue/CYPACK-618), CYPACK-640)
