...
### Added
- **Scoped environments for agent sessions** — You can now define reusable environment bundles as JSON files in `~/.cyrus/environments/<name>.json` and bind one to a session by writing `env=<name>` or `[env=<name>]` in an issue description. An environment can scope the session's system prompt, allowed/disallowed tools, MCP config paths, sandbox filesystem permissions, plugins, and skills, and can independently specify `repositories` (extra repos granted read-only access via their on-disk paths) and `gitWorktrees` (0 or more repo IDs for which git worktrees should be created — defaults to the routed repositories when omitted). Once bound, the environment persists across restarts — resumed sessions reapply the same scoping. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
