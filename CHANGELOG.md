...
### Added
- **Scoped environments for agent sessions** — You can now define reusable environment bundles (system prompt, allowed/disallowed tools, MCP config paths, sandbox filesystem permissions, plugins, skills) as JSON files in `~/.cyrus/environments/<name>.json`, and bind one to a session by writing `env=<name>` or `[env=<name>]` in an issue description. Once bound, the environment persists across restarts — resumed sessions reapply the same scoping. ([CYPACK-1130](https://linear.app/ceedar/issue/CYPACK-1130))
