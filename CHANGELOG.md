...
- Headless deployments can now change the default Claude model without redeploying or risking existing configuration: `POST /api/update/claude-model` (authenticated with `CYRUS_API_KEY` as a Bearer token, body `{"model": "sonnet"}`) updates `claudeDefaultModel` in place, unlike `/api/update/cyrus-config` which requires resending the entire configuration.
- Issues can now route to the correct repository based on their Linear project even when no repository has `projectKeys` configured for it. Since Linear exposes no structured project-to-repository link, project-based routing falls back to fuzzy-matching the Linear project name against each configured repository's name and GitHub/GitLab URL, only routing when the best match is confident and unambiguous.
- Headless deployments can now clone and register repositories into `config.json` in one merge-safe call: `POST /api/update/repositories` (authenticated with `CYRUS_API_KEY` as a Bearer token, body `{"repositories": [{"url": "...", "projectKeys": ["..."]}]}`) — unlike `/api/update/repository`, which only clones a repo to disk and never updates the routable repository list.
- Repositories no longer need to be registered by hand, one at a time. `POST /api/update/repositories/discover` (authenticated with `CYRUS_API_KEY` as a Bearer token, body `{"githubOwner": "your-org", "dryRun": true}`) looks up the connected Linear workspace's projects and a GitHub owner's repos live, fuzzy-matches each project to a repo, and clones + registers the confident matches — leaving ambiguous or unmatched projects for manual configuration instead of guessing. Pass `dryRun: true` to preview the matches without changing anything.
- An issue whose repository was routed (and cached) before that repository existed in configuration no longer stays stuck on the wrong repo forever. `POST /api/update/issue-repository-cache/clear` (authenticated with `CYRUS_API_KEY` as a Bearer token, body `{"issueId": "SWARM-114"}` — accepts either a Linear issue identifier or its internal id, or `{"all": true}` to reset every cached routing) clears the cached mapping so the next webhook for that issue re-routes from scratch.
- A new `railwayMcpConfigs` config option lets Cyrus load a Railway MCP server (deploy, logs, variables, domains) into agent sessions. Unlike the per-platform `linearMcpConfigs`/`githubMcpConfigs`/`slackMcpConfigs` options, it's applied across every session type — Linear, GitHub/GitLab, and Slack chat alike — so Railway tooling is available regardless of which platform triggered the session.
