...
### Changed
- Slack bot token is now read from the `SLACK_BOT_TOKEN` environment variable instead of the `X-Slack-Bot-Token` HTTP header on every request, improving security. Falls back to the header during the transition period with a warning log. ([CYPACK-824](https://linear.app/ceedar/issue/CYPACK-824), [#876](https://github.com/ceedaragents/cyrus/pull/876))
