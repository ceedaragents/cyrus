...
### Changed
- **Consolidated single-instance architecture** - Completed the consolidation from per-repository Maps to single-instance `IssueTrackerService` and `AgentSessionManager`. All agent sessions now track their repository context via the `repositoryContext` field, simplifying the architecture while maintaining full isolation between repositories. This change was completed across CYPACK-591, CYPACK-593, and CYPACK-594. All 222 tests passing. ([CYPACK-594](https://linear.app/ceedar/issue/CYPACK-594))
- Added CLI platform mode support to enable in-memory issue tracking for testing and development ([CYPACK-509](https://linear.app/ceedar/issue/CYPACK-509))
