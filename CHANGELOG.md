...
### Added
- **OpenTelemetry logs sink** — Every Cyrus log record can now also be shipped to an OTLP/HTTP collector alongside the existing stdout/stderr output. Configure via the new `telemetry` block in `~/.cyrus/config.json` or standard `OTEL_*` environment variables (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`, `OTEL_SDK_DISABLED`, etc.). Log body, severity, component, and session/issue/repository context are emitted as structured attributes. Uses OTLP/HTTP with protobuf encoding for broad collector compatibility (including Grafana Cloud, which rejects JSON-encoded OTLP). OpenTelemetry exporter errors (auth failures, network issues) are surfaced to stdout with an `[OTel]` prefix; set `OTEL_LOG_LEVEL=DEBUG` for verbose diagnostics. Disabled by default — no overhead when no endpoint is configured. By default only `WARN`/`ERROR` logs are forwarded — `INFO`/`DEBUG` prose stays on stdout only — and the floor is adjustable via `telemetry.minLogLevel` (`"DEBUG" | "INFO" | "WARN" | "ERROR"`) when richer traces are needed. Session lifecycle is emitted as structured events (`session.started`, `session.resumed`, `session.completed`, `session.stopped`, `session.failed`) with typed attributes (runner, model, duration, token counts, stop reason, error class) so Grafana dashboards can query `{event_name="session.completed"}` directly without regexing log bodies. New sessions and resumed sessions are distinguished so dashboards can track conversation continuity, and user-requested stops are separated from failures. Log attributes are scrubbed of common secret shapes (keys matching `token`, `secret`, `password`, `bearer`, `authorization`, `cookie`, `apiKey`, `clientSecret`, etc. across camelCase/snake_case/dotted paths) before being exported, so structured context can be logged without leaking credentials to the collector. Shutdown is bounded by a configurable `telemetry.shutdownTimeoutMs` (default `3000`) so an unreachable collector cannot hang process exit. ([CYPACK-1095](https://linear.app/ceedar/issue/CYPACK-1095), [#1122](https://github.com/ceedaragents/cyrus/pull/1122))
