...
### Fixed
- **Improved task list formatting in Linear activities** - Task lists now display as consolidated checklists instead of individual disconnected items, with bold subjects for better readability. When multiple tasks are created or updated in rapid succession, they are batched into a single activity showing the full task list. Subsequent task references show the task name instead of just a number like "#1". ([CYPACK-794](https://linear.app/ceedar/issue/CYPACK-794), [#845](https://github.com/ceedaragents/cyrus/pull/845))
