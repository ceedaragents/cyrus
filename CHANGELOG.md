...
- **Cyrus MCP tools now run on the built-in server endpoint** - Cyrus tools are now served via Fastify MCP on the same configured server port, improving MCP tool availability through a single endpoint, and cyrus-tools MCP requests now require `Authorization: Bearer <CYRUS_API_KEY>`. ([CYPACK-817](https://linear.app/ceedar/issue/CYPACK-817), [#870](https://github.com/ceedaragents/cyrus/pull/870))
