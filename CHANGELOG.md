...
### Changed
- **Updated Claude SDK dependencies** - Updated `@anthropic-ai/claude-agent-sdk` to v0.2.52 and `@anthropic-ai/sdk` to v0.78.0. New SDK features include `task_progress` events for real-time background agent progress reporting, `supportsEffort`/`supportedEffortLevels`/`supportsAdaptiveThinking` model capability fields, `ConfigChange` hook events for auditing settings changes, and fixes for memory growth in long-running sessions and `session.close()` breaking `resumeSession()`. See [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for details. ([CYPACK-844](https://linear.app/ceedar/issue/CYPACK-844), [#897](https://github.com/ceedaragents/cyrus/pull/897))
