...
### Changed
- **Updated `@anthropic-ai/claude-agent-sdk` to v0.2.121 and `@anthropic-ai/sdk` to v0.91.1** — Bumps the bundled Claude Code binary through v2.1.121; removes `LSP` from `availableTools` (no longer emitted by Claude Code's init block). New SDK features across v0.2.118–0.2.121: `managedSettings` for in-memory policy-tier settings, `forwardSubagentText` for subagent text deltas, `skills` option for controlling which Skills load, and `updatedToolOutput` in `PostToolUseHookSpecificOutput`. See [SDK changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for details. ([CYPACK-1146](https://linear.app/ceedar/issue/CYPACK-1146), [#1166](https://github.com/cyrusagents/cyrus/pull/1166))
